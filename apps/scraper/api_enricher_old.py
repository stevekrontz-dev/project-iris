...
def search_openalex(name: str, affiliation: str = "Kennesaw") -> Optional[Dict]:
    """Search OpenAlex for an author by name and affiliation."""
    # Build search query - OpenAlex uses different filter syntax
    ...
    if not data or not data.get('results'):
        # Try without affiliation filter
    ...
    # Find best match (prefer Kennesaw affiliation)
    for result in data['results']:
        institutions = result.get('last_known_institutions') or []
        for inst in institutions:
            if affiliation.lower() in (inst.get('display_name') or '').lower():
                return result

    # Fall back to first result if name matches well
    first = data['results'][0]
    if name.lower() in (first.get('display_name') or '').lower():
        return first
...
def enrich_from_semantic_scholar(name: str) -> Optional[Dict]:
    """Get full enrichment data from Semantic Scholar."""
    ...
    # Check if affiliation matches Kennesaw
    affiliations = details.get('affiliations', []) or []
    is_kennesaw = any('kennesaw' in (a or '').lower() for a in affiliations)

    return {
        'scholar_id': f"s2:{author_id}",
        'name': details.get('name', ''),
        'affiliation': affiliations[0] if affiliations else '',
        'interests': [],  # S2 doesn't provide research interests
        'h_index': details.get('hIndex', 0),
        'i10_index': 0,  # S2 doesn't provide i10
        'total_citations': details.get('citationCount', 0),
        'publication_count': details.get('paperCount', 0),
        'publications': publications,
        'source': 'semantic_scholar',
        'affiliation_match': is_kennesaw
    }
...
def search_orcid(first_name: str, last_name: str) -> Optional[str]:
    """Search ORCID for a researcher, return ORCID iD if found."""
    ...
    # Return first ORCID iD
    first_result = data['result'][0]
    return first_result.get('orcid-identifier', {}).get('path')
...
def get_orcid_works(orcid_id: str) -> List[Dict]:
    ...
    for group in data.get('group', [])[:MAX_PUBLICATIONS]:
        ...
        # Handle all the nested nulls in ORCID data
...
def enrich_from_orcid(first_name: str, last_name: str) -> Optional[Dict]:
    """Get enrichment data from ORCID."""
    ...
    return {
        ...
        'interests': [],
        'h_index': 0,  # ORCID doesn't provide h-index
        'i10_index': 0,
        'total_citations': 0,
...
def merge_scholar_data(existing: Optional[Dict], new_data: Dict) -> Dict:
    ...
    # Merge publications (dedupe by DOI or title)
...
def main():
    ...
    # Load existing output or use input
    ...
    log('INFO', '=' * 50)
    log('INFO', f'Done! {enriched_count} faculty enriched')
